...
metadata$Treatment = metadata$Group
...
se = SummarizedExperiment(assays=as.matrix(seDF), rowData=txann, colData=metadata)
...
par(las=2)
    barplot(colSums(assay(se)), main="Num mapped reads")
    abline(h=2e06, col="red")
dev.off()

### rRNA analysis ###
# se.ann = txann[match(rownames(se), txann$gene_id), ]
rRNA.idx = rowData(se)$gene_biotype == 'rRNA'
rRNA.se = se[rRNA.idx, ]
...
pdf(file="QA-QC-plots.pdf", width=16, height=8)
	...
	mycolors = nice_colors[1:length(levels(dds$Treatment))]
	...
	#MDS plots:
	d = dist(t(cpm(dds, log=T)))
	fit = cmdscale(d, eig=T, k=3)
	par(mfrow=c(1,2))
	plot(x=fit$points[,1], y=fit$points[,2], xlab="Coordinate 1", ylab="Coordinate 2", main="Metric MDS, log2 CPM", type='n',
	...
	legend('topleft', col=mycolors, legend=names(mycolors), pch=20, pt.cex=2)
	plot(x=fit$points[,1], y=fit$points[,3], xlab="Coordinate 1", ylab="Coordinate 3", main="Metric MDS, log2 CPM", type='n',
	...
	#Hclust:
	par(mfrow=c(1,2))
	hc = hclust(dist(t(cpm(dds, log=T))), method='ward.D2')
	plot(hc, main="Hclust with Euclidean distance and Ward.D2 clustering")
	hc = hclust(as.dist(1-abs(cor(cpm(dds, log=T)))), method='ward.D2')
	...
	library(pvclust)
	cluster.bootstrap = pvclust(cpm(dds, log=T), nboot=10, method.dist="abscor")
...
"
-Things generally look pretty good, one sample is a bit of an outlier maybe.
"
...
keep = rowSums(cpm(dds) > 3) >= min(table(dds$Treatment))
table(keep)
...
pdf(file="QA-QC-filtered-plots.pdf", width=16, height=8)
	...
	#Hclust:
	par(mfrow=c(1,2))
	hc = hclust(dist(t(cpm(dds2, log=T))), method='ward.D2')
	plot(hc, main="Hclust with Euclidean distance and Ward.D2 clustering")
	hc = hclust(as.dist(1-abs(cor(cpm(dds2, log=T)))), method='ward.D2')
	plot(hc, main="Hclust with 1-abs(correlation) distance and Ward.D2 clustering")
dev.off()
...
dds2$Treatment = relevel(droplevels(dds2$Treatment), ref="Uninfected")
...
all_contrasts = list(Ap_vs_Uninfected = c("Treatment", "Ap", "Uninfected"),
					 Bb_vs_Uninfected = c("Treatment", "Bb", "Uninfected"),
					 KIRA6.Uninfected_vs_Uninfected = c("Treatment", "KIRA6_Uninfected", "Uninfected"),
					 KIRA6.Ap_vs_KIRA6_Uninfected = c("Treatment", "KIRA6_Ap", "KIRA6_Uninfected"),
					 KIRA6.Bb_vs_KIRA6_Uninfected = c("Treatment", "KIRA6_Bb", "KIRA6_Uninfected"),
					 Ap_vs_KIRA6.Ap = c("Treatment", "Ap", "KIRA6_Ap"),
					 Bb_vs_KIRA6.Bb = c("Treatment", "Bb", "KIRA6_Bb"),
					 Ap_vs_Bb = c("Treatment", "Ap", "Bb"),
					 KIRA6.Ap_vs_KIRA6.Bb = c("Treatment", "KIRA6_Ap", "KIRA6_Bb")
)
...
process_contrast = function(contrast, all_contrasts, dds2){
	...
	#pdf(file=paste("./02-Plots/", contrast, '.pdf', sep=''), width=11, height=11)
		...
		anno = data.frame(Treatment=colData(dds2)[dds2$Treatment %in% l,]$Treatment)
		rownames(anno) = rownames(colData(dds2))[dds2$Treatment %in% l]
		#anno <- as.data.frame(colData(dds2)[dds2$Treatment %in% l])		
        ...
        showRownames=F
        if(length(DEGenes) < 50){showRownames=T}
		if(length(DEGenes) > 1){
