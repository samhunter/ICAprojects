---
title: "Microbial Community Analysis: Strawberry Microbiome 16s rRNA - Without plant ASVs"
author: "Sam Hunter"
...
#Drop plants:
ps.noPlant = subset_taxa(ps, Kingdom != "Plantae")

```
#### Dataset with plant derived ASVs

```{r wplant, echo=FALSE}
ps

```
#### Dataset without plant derived ASVs

```{r woplant, echo=FALSE}
ps.noPlant

```


#### Reads lost per sample after excluding plant ASVs

```{r p000.01_ReadsLost_by_SampleGroup, echo=FALSE, fig.height=8, fig.width=32, fig.align="center"}
df = data.frame(ReadsLost=rowSums(otu_table(ps)) - rowSums(otu_table(ps.noPlant)),
                PctReadsLost = 100*(rowSums(otu_table(ps)) - rowSums(otu_table(ps.noPlant)))/rowSums(otu_table(ps)))
df = data.frame(df, sample_data(ps)[rownames(df),c("Pathogen","SampleType","SampleGroup", "Field", "group")])

# Boxplot, percent reads lost by SampleGroup
ggplot(df, aes(x = SampleGroup, y = PctReadsLost, color = group)) +
  ...
  ggtitle("Percent reads lost by SampleGroup after excluding Plant derived ASVs")

```

#### All subsequence plots have been created without plant derived ASVs
```{r p000.02_drop_plant, echo=FALSE}
ps = ps.noPlant
```
